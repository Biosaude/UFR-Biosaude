...
export function ExecutiveOverview({ data, onOpenModule, onFiltersChange, resetSignal }: { data: ModuleStore; onOpenModule: (module: SourceModule) => void; onFiltersChange: (filters: Record<string, string[]>) => void; resetSignal: number }) {
  ...
  return <main className="dashboard executive-overview">
    ...
    <section className="panel overview-table"><div className="panel-title"><div><h3>Tabela Executiva Resumida</h3><p>Rastreabilidade por módulo e atualização</p></div><TableProperties/></div><div className="table-scroll"><table><thead><tr><th>Módulo</th><th>Valor</th><th>Registros</th><th>Período</th><th>Arquivo / aba</th><th>Última atualização</th><th>Responsável</th><th>Status</th></tr></thead><tbody>{executiveOverviewData.map((item) => <tr key={item.module}><td>{item.module}</td><td>{item.loaded ? money(item.total) : 'Aguardando inclusão da base de dados'}</td><td>{item.loaded ? item.filteredCount?.toLocaleString('pt-BR') : '—'}</td><td>{item.period ?? 'Informação não disponível na base de dados'}</td><td>{item.report ? `${item.report.fileName} • ${item.report.sheetName}` : '—'}</td><td>{item.updated?.toLocaleString('pt-BR') ?? '—'}</td><td>{item.updatedBy ?? '—'}</td><td>{item.loaded ? 'Disponível' : 'Aguardando base'}</td></tr>)}</tbody></table></div></section>
  </main>;
}
