...
const filterGroups = [
  ...
  { label: 'Localização', keys: ['hospitalState', 'clientState', 'hospital'] },
  { label: 'Comercial', keys: ['representative', 'client', 'doctor'] },
  { label: 'Produto', keys: ['brand', 'product', 'topic'] },
  { label: 'Processo', keys: ['voucherType', 'voucherStatus', 'surgeryType', 'scheduleType'] },
];
...
export function Dashboard({ rows, columns, resetSignal }: { rows: DataRow[]; columns: string[]; resetSignal: number }) {
  ...
  return <main className="dashboard">
    <div className={`filter-bar ${showMore ? 'expanded' : ''}`}>
      ...
      {showMore && <div className="additional-filters">{additionalGroups.map((group) => <FilterGroup key={group.label} label={group.label} filters={group.filters} optionsFor={optionsFor} selections={selections} setFilter={setFilter}/>)}</div>}
    </div>
    ...
    <section className="kpi-section"><div className="section-heading"><h2>Visão executiva</h2><p>Indicadores consolidados para o recorte selecionado</p></div><div className="kpi-groups">
      <KpiGroup label="Financeiro"><Kpi icon={<WalletCards/>} label="Valor utilizado" value={money(metrics.total)} description="Valor consolidado" tooltip={money(metrics.total)}/><Kpi icon={<WalletCards/>} label="Valor médio" value={money(metrics.average)} description="Média por registro" tooltip={`Valor médio por registro: ${money(metrics.average)}`}/></KpiGroup>
      <KpiGroup label="Operacional"><Kpi icon={<BarChart3/>} label="Registros" value={filteredRows.length.toLocaleString('pt-BR')} description="Linhas no recorte"/><Kpi icon={<Building2/>} label="Hospitais" value={show(metrics.hospital)} description="Contagem distinta"/><Kpi icon={<UsersRound/>} label="Clientes" value={show(metrics.client)} description="Contagem distinta"/></KpiGroup>
      <KpiGroup label="Comercial"><Kpi icon={<UserRound/>} label="Representantes" value={show(metrics.representative)} description="Contagem distinta"/><Kpi icon={<Stethoscope/>} label="Médicos" value={show(metrics.doctor)} description="Contagem distinta"/><Kpi icon={<Tags/>} label="Marcas" value={show(metrics.brand)} description="Contagem distinta"/></KpiGroup>
    </div>
    </section>
    <section className="visual-grid">
      <article className="panel timeline"><PanelTitle title="Utilizado ao longo do tempo" description="Evolução mensal do valor utilizado" action="Mensal"/>{timeline.length ? <ResponsiveContainer width="100%" height={312}><BarChart data={timeline} onClick={(event) => event?.activePayload?.[0] && addVisualFilter('month', event.activePayload[0].payload.month)}><CartesianGrid vertical={false} stroke="#e8edf0"/><XAxis dataKey="name" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false} tickFormatter={(value) => `${value / 1000}k`}/><Tooltip formatter={(value) => money(Number(value))}/><Bar dataKey="value" fill="#1d527e" radius={[5, 5, 0, 0]}/></BarChart></ResponsiveContainer> : <Unavailable/>}</article>
      <article className="panel"><PanelTitle title="Composição por marca" description="Participação no valor utilizado" action="Marca"/>{participation.length ? <div className="donut-wrap"><ResponsiveContainer width="55%" height={275}><PieChart><Pie data={participation} dataKey="value" nameKey="name" innerRadius={72} outerRadius={104} paddingAngle={2} onClick={(item) => addVisualFilter('brand', item.name)}>{participation.map((_, index) => <Cell key={index} fill={colors[index]}/>)}</Pie><Tooltip formatter={(value) => money(Number(value))}/></PieChart></ResponsiveContainer><div className="legend">{participation.map((item, index) => <button key={item.name} onClick={() => addVisualFilter('brand', item.name)}><i style={{ background: colors[index] }}/><span>{item.name}</span><b>{money(item.value)}</b></button>)}</div></div> : <Unavailable/>}</article>
      <article className="panel ranking"><PanelTitle description="Ranking por valor utilizado" title={rankingFilter?.key === 'hospital' ? 'Hospitais em destaque' : 'Clientes em destaque'} action="Top 5"/>{ranking.length ? <div className="ranking-list">{ranking.map((item, index) => <button key={item.name} onClick={() => rankingFilter && addVisualFilter(rankingFilter.key, item.name)}><b>{String(index + 1).padStart(2, '0')}</b><span>{item.name}</span><strong>{money(item.value)}</strong><i style={{ width: ranking[0].value ? `${(item.value / ranking[0].value) * 100}%` : '0%' }}/></button>)}</div> : <Unavailable/>}</article>
    </section>
...
function KpiGroup({ label, children }: { label: string; children: React.ReactNode }) { return <div className="kpi-group"><span>{label}</span><div>{children}</div></div>; }
...
function PanelTitle({ title, description, action }: { title: string; description: string; action?: string }) { return <div className="panel-title"><div><h3>{title}</h3><p>{description}</p></div>{action && <button>{action}<ChevronDown size={14}/></button>}</div>; }
