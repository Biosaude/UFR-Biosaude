...
type FilterDefinition = { key: string; label: string; source?: UtilizadoColumnKey; temporal?: 'year' | 'quarter' | 'month'; column?: string; modules?: Array<'Utilizado' | 'Faturado'> };
...
const filterGroups = [
  ...
  { label: 'Comercial', keys: ['representative', 'client', 'doctor', 'patient'] },
  { label: 'Produto', keys: ['brand', 'product', 'topic', 'productCode'] },
  { label: 'Processo', keys: ['voucherType', 'voucherStatus', 'scheduleType'] },
];

export function Dashboard({ rows, columns, resetSignal, module }: { rows: DataRow[]; columns: string[]; resetSignal: number; module: 'Utilizado' | 'Faturado' }) {
  ...
  const metrics = useMemo(() => {
    ...
    return { total, quantity: quantities.length ? quantities.reduce((sum, value) => sum + value, 0) : null, average: total !== null && filteredRows.length ? total / filteredRows.length : null, missingDates: filteredRows.length - datedRows.length, negativeCount: negatives.length, negativeTotal: negatives.reduce((sum, value) => sum + value, 0), zeroCount: values.filter((value) => value === 0).length, client: distinct('client'), hospital: distinct('hospital'), doctor: distinct('doctor'), representative: distinct('representative'), brand: distinct('brand'), company: distinct('company'), product: distinct('product'), topic: distinct('topic') };
  }, [filteredRows, filters, valueCol, quantityCol, dateCol]);
  const effectiveMetric: MetricKey = module === 'Faturado' ? 'value' : metric;
  ...
  return <main className="dashboard">
    ...
    <section className="kpi-section"><div className="section-heading"><h2>Visão executiva</h2><p>Indicadores consolidados para o recorte selecionado</p></div><div className="kpi-groups">
      <KpiGroup label="Principais indicadores" featured><Kpi source={module} icon={<WalletCards/>} label={module === 'Faturado' ? 'Valor faturado' : 'Valor utilizado'} value={money(metrics.total)} description="Valor consolidado" tooltip={money(metrics.total)}/>{module === 'Utilizado' && <Kpi source={module} icon={<Tags/>} label="Quantidade utilizada" value={number(metrics.quantity)} description="Soma das unidades registradas" tooltip="Soma das unidades registradas na coluna Quantidade Utilizada."/>}<Kpi source={module} icon={<BarChart3/>} label="Registros" value={filteredRows.length.toLocaleString('pt-BR')} description="Linhas no recorte"/>{module === 'Faturado' && <Kpi source={module} icon={<WalletCards/>} label="Valor médio" value={money(metrics.average)} description="Valor médio por registro" tooltip="Valor médio por registro"/>}</KpiGroup>
...
function ChartControls({ metric, onMetric, grain, onGrain, allowQuantity }: { metric: MetricKey; onMetric: (value: MetricKey) => void; grain: TimeGrain; onGrain: (value: TimeGrain) => void; allowQuantity: boolean }) { return <><select value={metric} onChange={(event) => onMetric(event.target.value as MetricKey)}><option value="value">Valor utilizado</option>{allowQuantity && <option value="quantity">Quantidade utilizada</option>}</select><select value={grain} onChange={(event) => onGrain(event.target.value as TimeGrain)}><option value="month">Mensal</option><option value="quarter">Trimestral</option><option value="year">Anual</option></select></>; }
