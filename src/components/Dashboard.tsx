...
import { normalizeName, resolveRecebidoColumns, resolveUtilizadoColumns, toDate, toNumber } from '../services/workbook';

type SelectionState = Record<string, string[]>;
type MetricKey = 'value' | 'quantity';
type TimeGrain = 'month' | 'quarter' | 'year';
type ModuleName = 'Utilizado' | 'Faturado' | 'Recebido';
type FilterDefinition = { key: string; label: string; source?: string; temporal?: 'year' | 'quarter' | 'month'; column?: string; modules?: ModuleName[] };
...
const filterDefinitions: FilterDefinition[] = [
  ...
  { key: 'productCode', label: 'Código Produto', source: 'productCode', modules: ['Faturado'] },
  { key: 'patient', label: 'Paciente', source: 'patient', modules: ['Faturado'] },
  { key: 'receivedPatient', label: 'Paciente', source: 'patient', modules: ['Recebido'] },
  { key: 'bank', label: 'Banco', source: 'bank', modules: ['Recebido'] },
  { key: 'collectionLocation', label: 'Local de cobrança', source: 'collectionLocation', modules: ['Recebido'] },
  { key: 'cashDesk', label: 'Caixa', source: 'cashDesk', modules: ['Recebido'] },
  { key: 'scheduleType', label: 'Tipo de Agendamento', source: 'scheduleType', modules: ['Utilizado'] },
  { key: 'voucherType', label: 'Tipo do Vale', source: 'voucherType', modules: ['Utilizado'] },
  { key: 'voucherStatus', label: 'Situação do Vale', source: 'voucherStatus', modules: ['Utilizado'] },
];
const colors = ['#1d527e', '#4d7899', '#7699af', '#98b1be', '#d59053', '#79a28c'];
const months = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];
const filterGroups = [
  { label: 'Tempo', keys: ['year', 'quarter', 'month'] },
  { label: 'Empresa e localização', keys: ['company', 'hospitalState', 'clientState', 'hospital', 'bank', 'collectionLocation', 'cashDesk'] },
  { label: 'Comercial', keys: ['representative', 'client', 'doctor', 'patient', 'receivedPatient'] },
  { label: 'Produto', keys: ['brand', 'product', 'topic', 'productCode'] },
...
export function Dashboard({ rows, columns, resetSignal, module, sharedFilters = {}, onSharedFiltersChange }: { rows: DataRow[]; columns: string[]; resetSignal: number; module: ModuleName; sharedFilters?: SelectionState; onSharedFiltersChange?: (filters: SelectionState) => void }) {
  ...
  const columnMap = useMemo<Record<string, string | undefined>>(() => {
    if (module !== 'Recebido') return resolveUtilizadoColumns(columns);
    const received = resolveRecebidoColumns(columns);
    return { ...received, surgeryDate: received.receiptDate, totalValue: received.receivedValue, billingClient: received.client, mainRepresentative: received.representative };
  }, [columns, module]);
  ...
  const metrics = useMemo(() => {
    ...
    const datedRows = dateCol ? filteredRows.filter((row) => toDate(row[dateCol])) : [];
    const negatives = valueCol ? filteredRows.map((row) => toNumber(row[valueCol])).filter((value): value is number => value !== null && value < 0) : [];
    const sumColumn = (key: string) => { const column = columnMap[key]; if (!column) return null; const items = filteredRows.map((row) => toNumber(row[column])).filter((value): value is number => value !== null); return items.length ? items.reduce((sum, value) => sum + value, 0) : null; };
    return { total, quantity: quantities.length ? quantities.reduce((sum, value) => sum + value, 0) : null, average: total !== null && filteredRows.length ? total / filteredRows.length : null, open: sumColumn('openValue'), discount: sumColumn('discountValue'), interest: sumColumn('interestValue'), missingDates: filteredRows.length - datedRows.length, negativeCount: negatives.length, negativeTotal: negatives.reduce((sum, value) => sum + value, 0), zeroCount: values.filter((value) => value === 0).length, client: distinct('client'), hospital: distinct('hospital'), doctor: distinct('doctor'), representative: distinct('representative'), brand: distinct('brand'), company: distinct('company'), product: distinct('product'), topic: distinct('topic') };
  }, [filteredRows, filters, valueCol, quantityCol, dateCol, columnMap]);
  const effectiveMetric: MetricKey = module !== 'Utilizado' ? 'value' : metric;
  const measureCol = effectiveMetric === 'value' ? valueCol : quantityCol;
  ...
  const compositionFilter = filters.find((filter) => filter.key === compositionKey) ?? filters.find((filter) => !filter.temporal);
  ...
  const formatMeasure = (value: number | null) => effectiveMetric === 'value' ? money(value) : number(value);
  const download = () => { const worksheet = XLSX.utils.json_to_sheet(searched); const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, worksheet, module); XLSX.writeFile(workbook, `${module.toLowerCase()}-filtrado.xlsx`); };
  ...
  return <main className="dashboard">
    ...
    <section className="kpi-section"><div className="section-heading"><h2>Visão executiva</h2><p>Indicadores consolidados para o recorte selecionado</p></div><div className="kpi-groups">
      <KpiGroup label="Principais indicadores" featured><Kpi source={module} icon={<WalletCards/>} label={module === 'Faturado' ? 'Valor faturado' : module === 'Recebido' ? 'Valor recebido' : 'Valor utilizado'} value={money(metrics.total)} description="Valor consolidado" tooltip={money(metrics.total)}/>{module === 'Utilizado' && <Kpi source={module} icon={<Tags/>} label="Quantidade utilizada" value={number(metrics.quantity)} description="Soma das unidades registradas" tooltip="Soma das unidades registradas na coluna Quantidade Utilizada."/>}<Kpi source={module} icon={<BarChart3/>} label="Registros" value={filteredRows.length.toLocaleString('pt-BR')} description="Linhas no recorte"/>{module !== 'Utilizado' && <Kpi source={module} icon={<WalletCards/>} label="Valor médio" value={money(metrics.average)} description="Valor médio por registro" tooltip="Valor médio por registro"/>}</KpiGroup>
      <KpiGroup label="Relacionamento"><Kpi source={module} icon={<UsersRound/>} label="Clientes" value={show(metrics.client)} description="Contagem distinta"/><Kpi source={module} icon={<Building2/>} label="Hospitais" value={show(metrics.hospital)} description="Contagem distinta"/><Kpi source={module} icon={<Stethoscope/>} label="Médicos" value={show(metrics.doctor)} description="Contagem distinta"/><Kpi source={module} icon={<UserRound/>} label="Representantes" value={show(metrics.representative)} description="Contagem distinta"/></KpiGroup>
      {module === 'Recebido' ? <KpiGroup label="Financeiro"><Kpi source={module} icon={<Building2/>} label="Empresas" value={show(metrics.company)} description="Contagem distinta"/><Kpi source={module} icon={<WalletCards/>} label="Valor em aberto" value={money(metrics.open)} description="Soma de Vr.aberto"/><Kpi source={module} icon={<WalletCards/>} label="Total de descontos" value={money(metrics.discount)} description="Soma de Vr.desconto"/><Kpi source={module} icon={<WalletCards/>} label="Total de juros" value={money(metrics.interest)} description="Soma de Vr.juros"/></KpiGroup> : <KpiGroup label="Produto"><Kpi source={module} icon={<Tags/>} label="Marcas" value={show(metrics.brand)} description="Contagem distinta"/><Kpi source={module} icon={<Tags/>} label="Produtos" value={show(metrics.product)} description="Contagem distinta"/><Kpi source={module} icon={<Tags/>} label="Tópicos do produto" value={show(metrics.topic)} description="Contagem distinta"/><Kpi source={module} icon={<Building2/>} label="Empresas" value={show(metrics.company)} description="Contagem distinta"/></KpiGroup>}
    </div>
    </section>
    {module === 'Faturado' && <section className="quality-strip"><span>Qualidade da base</span><div><Quality label="Sem Data da Cirurgia" value={number(metrics.missingDates)}/><Quality label="Valores negativos" value={number(metrics.negativeCount)} alert/><Quality label="Total negativo" value={money(metrics.negativeTotal)} alert/><Quality label="Valores zerados" value={number(metrics.zeroCount)}/></div></section>}
    <section className="visual-grid">
      <article className="panel timeline"><PanelTitle title={module === 'Faturado' ? 'Faturado conforme Data da Cirurgia' : module === 'Recebido' ? 'Evolução dos recebimentos' : `${effectiveMetric === 'value' ? 'Valor' : 'Quantidade'} utilizado ao longo do tempo`} description={module === 'Recebido' ? 'Evolução baseada exclusivamente em Dt.recebimento' : module === 'Faturado' && metrics.missingDates ? `${number(metrics.missingDates)} registros sem data permanecem nos totais` : 'Evolução baseada na Data da Cirurgia'} action={<ChartControls metric={effectiveMetric} onMetric={setMetric} grain={timeGrain} onGrain={setTimeGrain} allowQuantity={module === 'Utilizado'} valueLabel={module === 'Faturado' ? 'Valor faturado' : module === 'Recebido' ? 'Valor recebido' : 'Valor utilizado'}/>}/>{timeline.length ? <ResponsiveContainer width="100%" height={312}><BarChart data={timeline}><CartesianGrid vertical={false} stroke="#e8edf0"/><XAxis dataKey="name" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false} tickFormatter={(value) => effectiveMetric === 'value' ? `${value / 1000}k` : number(value)}/><Tooltip formatter={(value) => formatMeasure(Number(value))}/><Bar dataKey="value" fill="#1d527e" radius={[5, 5, 0, 0]}/></BarChart></ResponsiveContainer> : <Unavailable/>}</article>
      <article className="panel"><PanelTitle title={`Composição por ${compositionFilter?.label.toLowerCase() ?? 'marca'}`} description={`Participação por ${module === 'Faturado' ? 'valor faturado' : module === 'Recebido' ? 'valor recebido' : effectiveMetric === 'value' ? 'valor utilizado' : 'quantidade utilizada'}`} action={<DimensionSelect value={compositionKey} onChange={setCompositionKey} filters={filters}/>}/>{participation.length ? <div className="donut-wrap"><ResponsiveContainer width="55%" height={275}><PieChart><Pie data={participation} dataKey="value" nameKey="name" innerRadius={72} outerRadius={104} paddingAngle={2} onClick={(item) => compositionFilter && addVisualFilter(compositionFilter.key, item.name)}>{participation.map((_, index) => <Cell key={index} fill={colors[index]}/>)}</Pie><Tooltip formatter={(value) => formatMeasure(Number(value))}/></PieChart></ResponsiveContainer><div className="legend">{participation.map((item, index) => <button key={item.name} onClick={() => compositionFilter && addVisualFilter(compositionFilter.key, item.name)}><i style={{ background: colors[index] }}/><span>{item.name}</span><b>{formatMeasure(item.value)}</b></button>)}</div></div> : <Unavailable/>}</article>
      <article className="panel ranking"><PanelTitle description={`Ranking por ${module === 'Faturado' ? 'valor faturado' : module === 'Recebido' ? 'valor recebido' : effectiveMetric === 'value' ? 'valor utilizado' : 'quantidade utilizada'}`} title={`${rankingFilter?.label ?? 'Hospital'} em destaque`} action={<RankingControls dimension={rankingKey} onDimension={setRankingKey} limit={rankingLimit} onLimit={setRankingLimit} filters={filters}/>}/>{ranking.length ? <div className="ranking-list">{ranking.map((item, index) => <button key={item.name} onClick={() => rankingFilter && addVisualFilter(rankingFilter.key, item.name)}><b>{String(index + 1).padStart(2, '0')}</b><span>{item.name}</span><strong>{formatMeasure(item.value)}</strong><i style={{ width: ranking[0].value ? `${(item.value / ranking[0].value) * 100}%` : '0%' }}/></button>)}</div> : <Unavailable/>}</article>
    </section>
    <section className="panel table-panel"><div className="table-toolbar"><PanelTitle title="Detalhamento dos registros" description="Base analítica do recorte selecionado"/><div className="table-actions"><label><Search size={15}/><input placeholder="Pesquisar na tabela" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }}/></label><button className="secondary-button" onClick={download}><Download size={15}/> Exportar</button></div></div><div className="table-scroll"><table><thead><tr>{columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{searched.slice((page - 1) * 10, page * 10).map((row, index) => <tr key={index}>{columns.map((column) => <td key={column}>{display(row[column], column === valueCol)}</td>)}</tr>)}</tbody></table></div><div className="table-totals">{module === 'Utilizado' && <span>Quantidade utilizada <strong>{number(metrics.quantity)}</strong></span>}<span>{module === 'Recebido' ? 'Valor recebido' : 'Valor total'} <strong>{money(metrics.total)}</strong></span></div><div className="pagination"><span>{searched.length.toLocaleString('pt-BR')} registros</span><div><button disabled={page === 1} onClick={() => setPage(page - 1)}>Anterior</button><b>{page}</b><button disabled={page * 10 >= searched.length} onClick={() => setPage(page + 1)}>Próxima</button></div></div></section>
  </main>;
}
...
function display(value: DataRow[string], monetary = false) { if (value === null) return <span className="empty-cell">Não disponível</span>; if (value instanceof Date) return new Intl.DateTimeFormat('pt-BR').format(value); if (typeof value === 'number' && value < 0) return <span className="negative-value">({monetary ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Math.abs(value)) : new Intl.NumberFormat('pt-BR', { maximumFractionDigits: 2 }).format(Math.abs(value))})</span>; return String(value); }
function Quality({ label, value, alert }: { label: string; value: string; alert?: boolean }) { return <div className={alert ? 'alert' : ''}><span>{label}</span><strong>{value}</strong></div>; }
...
function Kpi({ icon, label, value, description, tooltip, source }: { icon: React.ReactNode; label: string; value: string; description: string; tooltip?: string; source: ModuleName }) { return <article className="kpi" title={tooltip}><div className="kpi-icon">{icon}</div><span>{label}</span><strong className={value.startsWith('Informação') ? 'unavailable-value' : ''}>{value}</strong><p>{description}</p><small>Fonte: base {source}</small></article>; }
...
function ChartControls({ metric, onMetric, grain, onGrain, allowQuantity, valueLabel }: { metric: MetricKey; onMetric: (value: MetricKey) => void; grain: TimeGrain; onGrain: (value: TimeGrain) => void; allowQuantity: boolean; valueLabel: string }) { return <><select value={metric} onChange={(event) => onMetric(event.target.value as MetricKey)}><option value="value">{valueLabel}</option>{allowQuantity && <option value="quantity">Quantidade utilizada</option>}</select><select value={grain} onChange={(event) => onGrain(event.target.value as TimeGrain)}><option value="month">Mensal</option><option value="quarter">Trimestral</option><option value="year">Anual</option></select></>; }
...
function RankingControls({ dimension, onDimension, limit, onLimit, filters }: { dimension: string; onDimension: (value: string) => void; limit: number; onLimit: (value: number) => void; filters: FilterDefinition[] }) { return <><DimensionSelect value={dimension} onChange={onDimension} filters={filters}/><select value={limit} onChange={(event) => onLimit(Number(event.target.value))}><option value={5}>Top 5</option><option value={10}>Top 10</option><option value={15}>Top 15</option><option value={20}>Top 20</option><option value={50}>Top 50</option></select></>; }
