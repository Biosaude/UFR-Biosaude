...
function App() {
  ...
  return <div className="app-shell">
    <header><div className="brand-mark">UFR</div><div className="heading"><p>INTELIGÊNCIA EXECUTIVA</p><h1>Utilizado <i/> <span>Faturado</span> <i/> <span>Recebido</span></h1><h2>Dashboard Executivo</h2></div><div className="header-actions"><div className="updated"><Clock3/><span>Última atualização<strong>{updated ? updated.toLocaleString('pt-BR') : 'Informação não disponível na base de dados'}</strong></span></div><button className="icon-button" aria-label="Atualizar" onClick={() => hiddenInput.current?.click()}><RefreshCw/></button><button className="secondary-button" onClick={() => {}}><Eraser/> Limpar filtros</button><button className="primary-button" onClick={openUpload}><Upload/> Atualizar base</button><input ref={hiddenInput} hidden type="file" accept=".xlsx" onChange={(e)=>e.target.files?.[0]&&chooseFile(e.target.files[0])}/></div></header>
    ...
    {rows.length ? <Dashboard rows={rows} columns={columns}/> : <EmptyState onUpload={openUpload}/>}<footer>UFR Intelligence <span>•</span> Dados exibidos exclusivamente a partir da base importada</footer>
